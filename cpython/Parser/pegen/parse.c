...
static stmt_ty sandbox_stmt_rule(Parser *p);
...
compound_stmt_rule(Parser *p)
{
    ...
    { // &'sandbox' sandbox_stmt
        ...
        D(fprintf(stderr, "%*c> compound_stmt[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "&'sandbox' sandbox_stmt"));
        stmt_ty sandbox_stmt_var;
        if (
            _PyPegen_lookahead_with_int(1, _PyPegen_expect_token, p, 513)  // token='sandbox'
            &&
            (sandbox_stmt_var = sandbox_stmt_rule(p))  // sandbox_stmt
        )
        {
            D(fprintf(stderr, "%*c+ compound_stmt[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "&'sandbox' sandbox_stmt"));
            _res = sandbox_stmt_var;
            goto done;
        }
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s compound_stmt[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "&'sandbox' sandbox_stmt"));
    }
    _res = NULL;
  done:
    D(p->level--);
...
sandbox_stmt_rule(Parser *p)
{
    ...
    { // 'sandbox' '(' named_expression ',' named_expression ')' ':' block
        ...
        asdl_seq* c;
        if (
            (_keyword = _PyPegen_expect_token(p, 513))  // token='sandbox'
            &&
            (_literal = _PyPegen_expect_token(p, 7))  // token='('
            &&
            (a = named_expression_rule(p))  // named_expression
